...
class MambaBlockLayer(nn.Module):
    def __init__(self, dim, nums, window_shape, group_size, direction, shift, operator=None, layer_id=0, n_layer=0):
        super(MambaBlockLayer, self).__init__()
...
class MambaBlock3D(nn.Module):
    def __init__(self, dim: int, depth: int, down_scales: list, window_shape, group_size, direction, shift=False,
        ...
        for idx in range(depth):
            self.encoder.append(MambaBlockLayer(dim, 1, window_shape, group_size, direction, shift[idx], operator, layer_id + idx * 2, n_layer))
        ...
        for idx in range(depth):
            self.decoder.append(MambaBlockLayer(dim, 1, window_shape, group_size, direction, shift[idx], operator, layer_id + 2 * (idx + depth), n_layer))
...
class Mamba_3D_Backbone(nn.Module):
    def __init__(self, model_cfg, input_channels, grid_size, **kwargs):
        ...
        self.linear_1 = MambaBlock3D(self.layer_dim[0], depths[0], layer_down_scales[0], self.window_shape[0],
        ...
        self.linear_2 = MambaBlock3D(self.layer_dim[1], depths[1], layer_down_scales[1], self.window_shape[1],
        ...
        #  [236, 236, 8] -> [236, 236, 4]
        self.linear_3 = MambaBlock3D(self.layer_dim[2], depths[2], layer_down_scales[2], self.window_shape[2],
        ...
        #  [236, 236, 4] -> [236, 236, 2]
        self.linear_4 = MambaBlock3D(self.layer_dim[3], depths[3], layer_down_scales[3], self.window_shape[3],
        ...
        self.linear_out = MambaBlockLayer(self.layer_dim[3], 1, [13, 13, 2], 256, direction=['x', 'y'], shift=shift,
...
#for argoverse
class LMamba_3D_Backbone_Sparse(nn.Module):
    def __init__(self, model_cfg, input_channels, grid_size, **kwargs):
        super().__init__()
        ...
        self.linear_bev1 = MambaBlockLayer(self.layer_dim[3], 1, [25, 25, 1], 512, direction=['x', 'y'], shift=shift,
                                      operator=self.linear_operator, layer_id=34, n_layer=self.n_layer)
        self.linear_bev2 = MambaBlockLayer(self.layer_dim[3], 1, [37, 37, 1], 1024, direction=['x', 'y'], shift=shift,
                                       operator=self.linear_operator, layer_id=36, n_layer=self.n_layer)
        self.linear_bev3 = MambaBlockLayer(self.layer_dim[3], 1, [51, 51, 1], 2048, direction=['x', 'y'], shift=shift,
